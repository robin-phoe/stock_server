...
def get_kline_simple(request):
    ...
    df['point_type'] = df['point_type'].apply(lambda x: 'n' if x == '' else x)
    df = df[["stock_id","open_price","close_price","low_price","high_price"]]
    df_min = df.groupby('stock_id').min()
    # 行转换为列表
    rows_list = df.values.tolist()
    kline_json = {}
    for row in rows_list:
        id = row[0]
        min_base_value = df_min.loc[id,"low_price"]
        val = [row[1] - min_base_value, row[2] - min_base_value , row[3]- min_base_value, row[4] - min_base_value]
        if id in kline_json:
